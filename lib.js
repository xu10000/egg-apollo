...
module.exports = {
  ...
  mergeConfig(app, remoteConfig) {
    ...
    if (app.config.apollo.printKeyValue) {
      this.printKeyValue(app, app.config);
    }
    return app.config;
  },
  getRemoteConfig(app) {
    const { apollo } = app.config;
    try {

      if (!apollo) {
        throw new Error('-----------配置文件没有app.config.apollo');
      }
      ...
      // !!! 合并本地和远程的配置
      return this.mergeConfig(app, remoteConfig);
    } catch (err) {
      app.logger.error('----------in catch 请求远程配置失败 ', err);
      process.exit();
    }
